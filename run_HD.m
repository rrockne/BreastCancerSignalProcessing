% Script that sets up the simulation for all healthy donnors (HD) and all cell subtypes. Results are 
...
outfile = 'Results_HD_All.mat'; 

num_cell_types = numel(cell_types);
num_samples = 32; % number of HD donors
...
for ii = 1:num_cell_types
    filename = ['casefile_HD_' cell_types{ii} '.csv'];
    Q = zeros(6);
    for jj = 1:num_samples
        donor_label = ['HD' num2str(jj)];
